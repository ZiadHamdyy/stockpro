 ...
 */
export const exportToPdf = (title: string, columns: any[][], body: any[], fileName: string, companyInfo?: any, footerRows?: any[][]) => {
    try {
        const { jsPDF } = jspdf;
        const doc = new jsPDF({
            orientation: 'p',
        ...
        const pageContent = (data: any) => {
            ...
            if (companyInfo) {
                 doc.setFontSize(10);
                 doc.text(companyInfo.name, doc.internal.pageSize.getWidth() - data.settings.margin.right, 15, { align: 'right' });
            }
            ...
            doc.text(`صفحة ${data.pageNumber} من ${pageCount}`, doc.internal.pageSize.getWidth() - data.settings.margin.right, doc.internal.pageSize.getHeight() - 10, { align: 'right' });
        };
