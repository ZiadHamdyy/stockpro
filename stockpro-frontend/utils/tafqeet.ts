...
const currencies: { [key: string]: { singular: string, plural: string, fraction: string, fractionPlural: string } } = {
...
export function tafqeet(number: number, currency: string = 'SAR'): string {
    ...
    if (m > 0) {
        ...
        else if (m >= 3 && m <= 10) result += convertThreeDigits(m) + ' ' + millions[3];
    ...
    if (t > 0) {
        ...
        else if (t >= 3 && t <= 10) result += convertThreeDigits(t) + ' ' + thousands[3];
    ...
    if (integerPart > 0) {
      result += ' ' + getCurrencyName(integerPart, currencyInfo.singular, currencyInfo.plural);
    }
    
    if (fractionalPart > 0) {
        if(integerPart > 0) result += ' و';
        result += convertThreeDigits(fractionalPart) + ' ' + getCurrencyName(fractionalPart, currencyInfo.fraction, currencyInfo.fractionPlural);
    }
