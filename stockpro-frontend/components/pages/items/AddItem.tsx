...
const AddItem: React.FC<AddItemProps> = ({ title, editingId, onNavigate }) => {
    const [itemData, setItemData] = useState<Partial<Item> & { groupId?: string; unitId?: string }>({
        code: '', barcode: '', name: '', purchasePrice: 0, salePrice: 0, stock: 0, reorderLimit: 0,
    });
    ...
    // API hooks
    const { data: itemsResponse, isLoading: itemsLoading } = useGetItemsQuery(undefined);
    const { data: itemGroupsResponse, isLoading: groupsLoading, error: groupsError } = useGetItemGroupsQuery(undefined);
    const { data: unitsResponse, isLoading: unitsLoading, error: unitsError } = useGetUnitsQuery(undefined);
    ...
    useEffect(() => {
        if (editingId !== null) {
            const foundItem = Array.isArray(items) ? items.find(item => item.id === editingId) : null;
            if (foundItem) {
                setItemData(foundItem);
                setIsReadOnly(true);
            }
        } else {
            const nextCode = Array.isArray(items) && items.length > 0
                ? (Math.max(...items.map(i => parseInt(i.code, 10) || 0)) + 1).toString()
                : '101';
            setItemData({
                code: nextCode, barcode: '', name: '', purchasePrice: 0, salePrice: 0, stock: 0, reorderLimit: 0,
            });
            setIsReadOnly(false);
        }
    }, [editingId, items]);
    ...
    const handleSave = async () => {
        if (!itemData.code || !itemData.name || !itemData.groupId || !itemData.unitId) {
        ...
        <div className="bg-white p-6 rounded-lg shadow">
            ...
            {(groupsLoading || unitsLoading) && (
                <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-md">
                    <div className="flex items-center">
                        ...
                        <span className="text-blue-700 text-sm">جاري تحميل البيانات...</span>
                    </div>
                </div>
            )}
            <form onSubmit={(e) => { e.preventDefault(); handleSave(); }} className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div>
                        <label htmlFor="groupId" className="block text-sm font-medium text-gray-700">المجموعة</label>
                        <select name="groupId" id="groupId" value={itemData.groupId || ''} onChange={handleChange} className={inputStyle} disabled={isReadOnly || groupsLoading} required>
                            <option value="">
                                {groupsLoading ? 'جاري تحميل المجموعات...' : 'اختر مجموعة...'}
                            </option>
                            {Array.isArray(itemGroups) ? itemGroups.map(group => <option key={group.id} value={group.id}>{group.name}</option>) : []}
                        </select>
                        {groupsError && (
                            <p className="text-red-500 text-sm mt-1">خطأ في تحميل المجموعات</p>
                        )}
                    </div>
                    <div>
                        <label htmlFor="unitId" className="block text-sm font-medium text-gray-700">الوحدة</label>
                        <select name="unitId" id="unitId" value={itemData.unitId || ''} onChange={handleChange} className={inputStyle} disabled={isReadOnly || unitsLoading} required>
                            <option value="">
                                {unitsLoading ? 'جاري تحميل الوحدات...' : 'اختر وحدة...'}
                            </option>
                            {Array.isArray(units) ? units.map(unit => <option key={unit.id} value={unit.id}>{unit.name}</option>) : []}
                        </select>
                        {unitsError && (
                 ...
                 <div className="mt-8 pt-6 border-t-2 border-gray-200 flex flex-col items-start space-y-4">
                    ...
                    <div className="flex items-center justify-start gap-2">
                        <button type="button" onClick={() => navigate('first')} disabled={!Array.isArray(items) || items.length === 0} className="p-2 bg-gray-200 rounded-md hover:bg-gray-300 disabled:opacity-50">الأول</button>
                        <button type="button" onClick={() => navigate('prev')} disabled={!Array.isArray(items) || items.length === 0} className="p-2 bg-gray-200 rounded-md hover:bg-gray-300 disabled:opacity-50">السابق</button>
                        <div className="px-4 py-2 bg-brand-blue-bg border-2 border-brand-blue rounded-md">
                           <span className="font-bold">{itemData.id ? `تعديل صنف` : `سجل جديد`}</span>
                        </div>
                        <button type="button" onClick={() => navigate('next')} disabled={!Array.isArray(items) || items.length === 0} className="p-2 bg-gray-200 rounded-md hover:bg-gray-300 disabled:opacity-50">التالي</button>
                        <button type="button" onClick={() => navigate('last')} disabled={!Array.isArray(items) || items.length === 0} className="p-2 bg-gray-200 rounded-md hover:bg-gray-300 disabled:opacity-50">الأخير</button>
                    </div>
                </div>
            </form>
        </div>
