...
import type { CompanyInfo, StoreVoucherItem, Store, StoreTransferVoucher } from '../../../types';
...
interface StoreTransferProps {
    ...
    onSave: (voucher: StoreTransferVoucher) => void;
    onDelete: (id: string) => void;
}

const DocumentHeader: React.FC<{ companyInfo: CompanyInfo }> = ({ companyInfo }) => (
...
const StoreTransfer: React.FC<StoreTransferProps> = ({ title, companyInfo, items: allItems, stores, vouchers, onSave, onDelete }) => {
    ...
    const handleNew = useCallback(() => {
        const nextIdNumber = vouchers.length > 0 ? Math.max(...vouchers.map(v => parseInt(v.id.split('-')[1]) || 0)) + 1 : 1;
        setCurrentIndex(-1);
        setItems([]);
        setVoucherDetails({
            id: `STV-${String(nextIdNumber).padStart(5, '0')}`,
            date: new Date().toISOString().substring(0, 10),
            fromStore: stores.length > 0 ? stores[0].name : '',
            toStore: stores.length > 1 ? stores[1].name : ''
        });
    ...
    useEffect(() => {
        if (currentIndex >= 0 && vouchers[currentIndex]) {
            ...
            setVoucherDetails({ id: v.id, date: v.date, fromStore: v.fromStore, toStore: v.toStore });
            setItems(v.items);
            setIsReadOnly(true);
        } else {
            handleNew();
    ...
    const handleItemChange = (index: number, field: keyof StoreVoucherItem, value: any) => {
    ...
    const handleSave = () => {
        ...
        const voucherData: StoreTransferVoucher = {
            id: voucherDetails.id,
            date: voucherDetails.date,
            fromStore: voucherDetails.fromStore,
            toStore: voucherDetails.toStore,
            items,
        };
        onSave(voucherData);
    ...
    const inputStyle = "block w-full bg-yellow-100 border-2 border-amber-500 rounded-md shadow-sm text-black placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-amber-500 py-3 px-4 disabled:bg-gray-200 disabled:cursor-not-allowed";
    const tableInputStyle = "text-center bg-transparent focus:outline-none focus:ring-1 focus:ring-amber-500 rounded p-1 w-full disabled:bg-transparent";
    ...
    return (
        <>
                ...
                <div className="border-2 border-amber-500 rounded-lg mb-4">
                    <div className="p-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
                            <input type="text" placeholder="رقم السند" className={inputStyle + " bg-gray-200"} value={voucherDetails.id} readOnly />
                            <input type="date" className={inputStyle} value={voucherDetails.date} onChange={e => setVoucherDetails({...voucherDetails, date: e.target.value})} disabled={isReadOnly} />
                            <select className={inputStyle} value={voucherDetails.fromStore} onChange={e => setVoucherDetails({...voucherDetails, fromStore: e.target.value})} disabled={isReadOnly}>
                                <option value="" disabled>من مخزن...</option>
                                {stores.map(s => <option key={s.id} value={s.name}>{s.name}</option>)}
                            </select>
                             <select className={inputStyle} value={voucherDetails.toStore} onChange={e => setVoucherDetails({...voucherDetails, toStore: e.target.value})} disabled={isReadOnly}>
                                <option value="" disabled>إلى مخزن...</option>
                                {stores.map(s => <option key={s.id} value={s.name}>{s.name}</option>)}
                            </select>
                        </div>
                    </div>
                    ...
                    <table className="min-w-full">
                        <thead className="bg-amber-500">
                            <tr className="divide-x divide-amber-400">
                                <th className="px-2 py-3 w-10 text-center text-sm font-semibold text-white uppercase">م</th>
                                <th className="px-2 py-3 w-32 text-right text-sm font-semibold text-white uppercase">رقم الصنف</th>
                                <th className="px-2 py-3 w-2/5 text-right text-sm font-semibold text-white uppercase">الصنف</th>
                                <th className="px-2 py-3 w-32 text-center text-sm font-semibold text-white uppercase">الوحدة</th>
                                <th className="px-2 py-3 w-32 text-center text-sm font-semibold text-white uppercase">الكمية</th>
                                <th className="px-2 py-3 w-16 text-center"></th>
                            </tr>
                        </thead>
                        <tbody ref={itemSearchRef}>
                            {items.map((item, index) => (
                                 <tr key={index} className="divide-x divide-gray-200 border-b border-gray-200 last:border-b-0 hover:bg-yellow-100 transition-colors duration-150">
                                    <td className="p-2 align-middle text-center">{index + 1}</td>
                                    <td className="p-2 align-middle"><input type="text" value={item.id} onChange={(e) => handleItemChange(index, 'id', e.target.value)} className={tableInputStyle} disabled={isReadOnly}/></td>
                                        ...
                                        <div className="flex items-center">
                                         ...
                                         {activeItemSearch?.index === index && filteredItems.length > 0 && !isReadOnly && (
                                            <div className="absolute z-10 w-full mt-1 bg-white border border-gray-300 rounded-md shadow-lg max-h-48 overflow-y-auto">
                                                {filteredItems.map((result, idx) => ( <div key={result.id} onClick={() => handleSelectItem(index, result)} className={`p-2 cursor-pointer ${idx === highlightedIndex ? 'bg-amber-500 text-white' : 'hover:bg-yellow-100'}`} onMouseEnter={() => setHighlightedIndex(idx)}>{result.name} ({result.id})</div>))}
                                            </div>
                                        )}
                                    </td>
                                    <td className="p-2 align-middle"><input type="text" value={item.unit} onChange={(e) => handleItemChange(index, 'unit', e.target.value)} className={tableInputStyle} disabled={isReadOnly}/></td>
                                    <td className="p-2 align-middle"><input type="number" value={item.qty} onChange={(e) => handleItemChange(index, 'qty', e.target.value)} ref={el => { if (el) qtyInputRefs.current[index] = el; }} className={tableInputStyle} disabled={isReadOnly}/></td>
                                    <td className="p-2 align-middle text-center"><button onClick={() => handleRemoveItem(index)} className="text-red-500 p-1 rounded-full hover:bg-red-100 hover:text-red-700 disabled:text-gray-400 disabled:hover:bg-transparent" disabled={isReadOnly}><TrashIcon className="w-5 h-5" /></button></td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
                    ...
                    <div className="flex justify-around items-center mt-8 text-center text-sm font-semibold">
                     ...
                     <div className="mt-8 pt-6 border-t-2 border-gray-200 flex flex-col items-center space-y-4">
                        <div className="flex justify-center gap-2 flex-wrap">
                            ...
                            <button onClick={handleSave} disabled={isReadOnly || items.length === 0} className="px-4 py-2 bg-amber-500 text-white rounded-md hover:bg-amber-600 font-semibold disabled:bg-gray-400">حفظ</button>
                            ...
                            <button onClick={() => window.print()} className="px-4 py-2 bg-gray-200 text-brand-dark rounded-md hover:bg-gray-300 font-semibold flex items-center"><PrintIcon className="mr-2 w-5 h-5"/> طباعة</button>
                        </div>
            ...
            <DataTableModal
                ...
                title="قائمة الأصناف"
                columns={[ { Header: 'الكود', accessor: 'id' }, { Header: 'الاسم', accessor: 'name' }, { Header: 'الرصيد', accessor: 'stock' }, { Header: 'الوحدة', accessor: 'unit' } ]}
                data={allItems}
            ...
            />
             <DataTableModal
                ...
                columns={[ { Header: 'الرقم', accessor: 'id' }, { Header: 'التاريخ', accessor: 'date' }, { Header: 'من مخزن', accessor: 'fromStore' }, { Header: 'إلى مخزن', accessor: 'toStore' } ]}
                data={vouchers}
