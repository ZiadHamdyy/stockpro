...
const StoreTransfer: React.FC<StoreTransferProps> = ({ title }) => {
  ...
  const lastInitializedRef = useRef<{ currentIndex: number; defaultToStore: string } | null>(null);
  ...
  const handleNew = useCallback(() => {
    ...
    setIsReadOnly(false);
  }, [stores, userStore, defaultToStore]);
  ...
  useEffect(() => {
    if (currentIndex >= 0 && vouchers[currentIndex]) {
      ...
      setVoucherDetails((prev) => {
        if (
          prev.id === newVoucherDetails.id &&
          prev.date === newVoucherDetails.date &&
          prev.fromStore === newVoucherDetails.fromStore &&
          prev.toStore === newVoucherDetails.toStore
        ) {
          return prev;
        }
        return newVoucherDetails;
      });
      ...
      // Use ref to prevent re-initialization if we've already initialized with the same values
      const shouldInitialize = !lastInitializedRef.current || 
        lastInitializedRef.current.currentIndex !== currentIndex ||
        lastInitializedRef.current.defaultToStore !== defaultToStore;
      ...
      if (shouldInitialize) {
        ...
        setVoucherDetails((prev) => {
          ...
          if (
            prev.id === newDetails.id &&
            prev.date === newDetails.date &&
            prev.fromStore === newDetails.fromStore &&
            prev.toStore === newDetails.toStore
          ) {
            return prev;
          }
          return newDetails;
        ...
        setItems(getEmptyItems());
        setIsReadOnly(false);
        lastInitializedRef.current = { currentIndex, defaultToStore };
      }
    } else if (currentIndex >= 0) {
      // Reset ref when viewing an existing voucher
      lastInitializedRef.current = null;
    }
  }, [currentIndex, vouchers, isLoadingVouchers, userStore, defaultToStore]);

  // Always keep fromStore synced with userStore when creating new vouchers
  useEffect(() => {
    if (currentIndex === -1 && userStore) {
      setVoucherDetails((prev) => {
        // Only update if fromStore doesn't match userStore.name
        if (prev.fromStore !== userStore.name) {
          return {
            ...prev,
            fromStore: userStore.name,
          };
  ...
  const handleScanSuccess = (barcode: string) => {
    ...
    if (foundItem) {
      ...
      setItems(newItems);
      setFocusedItemIndex(indexToFill);
