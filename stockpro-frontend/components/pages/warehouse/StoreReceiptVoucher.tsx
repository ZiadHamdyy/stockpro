...
import type { CompanyInfo, StoreVoucherItem, Branch, StoreReceiptVoucher as StoreReceiptVoucherType } from '../../../types';
...
interface StoreReceiptVoucherProps {
    ...
    branches: Branch[];
    vouchers: StoreReceiptVoucherType[];
    onSave: (voucher: StoreReceiptVoucherType) => void;
...
const StoreReceiptVoucher: React.FC<StoreReceiptVoucherProps> = ({ title, companyInfo, items: allItems, branches, vouchers, onSave, onDelete }) => {
    ...
    const handleNew = useCallback(() => {
        ...
        setItems([]);
        setVoucherDetails({
            id: `SRV-${String(nextIdNumber).padStart(5, '0')}`,
            date: new Date().toISOString().substring(0, 10),
            branch: branches.length > 0 ? branches[0].name : '',
        });
        setIsReadOnly(false);
    ...
    const handleSave = () => {
        ...
        const voucherData: StoreReceiptVoucherType = {
    ...
    const tableInputStyle = "text-center bg-transparent focus:outline-none focus:ring-1 focus:ring-brand-blue rounded p-1 w-full disabled:bg-transparent";
        ...
        <>
                ...
                <div className="border-2 border-brand-blue rounded-lg mb-4">
                    <div className="p-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                            ...
                            <select className={inputStyle} value={voucherDetails.branch} onChange={e => setVoucherDetails({...voucherDetails, branch: e.target.value})} disabled={isReadOnly}>
                    ...
                    <table className="min-w-full">
                        ...
                        <tbody ref={itemSearchRef}>
                            {items.map((item, index) => (
                                 <tr key={index} className="divide-x divide-gray-200 border-b border-gray-200 last:border-b-0 hover:bg-brand-blue-bg transition-colors duration-150">
                    ...
                    <div className="flex justify-around items-center mt-8 text-center text-sm font-semibold">
                     ...
                     <div className="mt-8 pt-6 border-t-2 border-gray-200 flex flex-col items-center space-y-4">
                        <div className="flex justify-center gap-2 flex-wrap">
                            ...
                            <button onClick={handleSave} disabled={isReadOnly || items.length === 0} className="px-4 py-2 bg-brand-green text-white rounded-md hover:bg-green-700 font-semibold disabled:bg-gray-400">حفظ</button>
            ...
            />
             <DataTableModal
                ...
                title="بحث عن إذن إضافة"
                columns={[ { Header: 'الرقم', accessor: 'id' }, { Header: 'التاريخ', accessor: 'date' }, { Header: 'الفرع', accessor: 'branch' } ]}
                data={vouchers}
