...
const StoreReceiptVoucher: React.FC<StoreReceiptVoucherProps> = ({ title }) => {
  ...
  }, [currentIndex, vouchers, isLoadingVouchers, branches]);
  ...
  const handleSave = async () => {
    // Ignore completely empty rows (id and name both missing)
  ...
  const tableInputStyle =
    "text-center bg-transparent focus:outline-none focus:ring-1 focus:ring-brand-blue rounded p-1 w-full disabled:bg-transparent";
          ...
          <table className="min-w-full">
            ...
            <tbody ref={itemSearchRef}>
              {items.map((item, index) => {
                ...
                <tr
                  key={index}
                  className={`divide-x divide-gray-200 border-b border-gray-200 last:border-b-0 hover:bg-brand-blue-bg transition-colors duration-150 ${isEmptyRow ? 'empty-row' : ''}`}
                >
