...
import { BarcodeIcon, PdfIcon, ListIcon, PrintIcon, SearchIcon, TrashIcon } from '../../icons';
import { tafqeet } from '../../../utils/tafqeet';
import type { CompanyInfo, InvoiceItem, Supplier, Invoice, User, Item, Safe, Bank } from '../../../types';
...
const PurchaseInvoice: React.FC<PurchaseInvoiceProps> = ({ title, vatRate, isVatEnabled, companyInfo, items: allItems, suppliers: allSuppliers, invoices, onSave, onDelete, currentUser, viewingId, onClearViewingId, setItems: setGlobalItems, safes, banks }) => {
    ...
    const [invoiceDetails, setInvoiceDetails] = useState({ invoiceNumber: '', invoiceDate: new Date().toISOString().substring(0, 10) });
    const [isReadOnly, setIsReadOnly] = useState(true);
    const [paymentTargetType, setPaymentTargetType] = useState<'safe' | 'bank'>('safe');
    ...
    const filteredSuppliers = supplierQuery ? allSuppliers.filter(s => s.name.toLowerCase().includes(supplierQuery.toLowerCase())) : allSuppliers;
    ...
    useEffect(() => {
        const handleClickOutside = (event: MouseEvent) => {
            ...
            if (supplierRef.current && !supplierRef.current.contains(event.target as Node)) setIsSupplierDropdownOpen(false);
        };
    ...
    const handleItemSearchKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
        ...
        switch (e.key) {
            case 'ArrowDown': e.preventDefault(); setHighlightedIndex(prev => (prev + 1) % filteredItems.length); break;
            case 'ArrowUp': e.preventDefault(); setHighlightedIndex(prev => (prev - 1 + filteredItems.length) % filteredItems.length); break;
            case 'Enter':
                e.preventDefault();
                if (highlightedIndex > -1 && filteredItems[highlightedIndex]) {
                    handleSelectItem(activeItemSearch.index, filteredItems[highlightedIndex]);
                }
                break;
            case 'Escape': e.preventDefault(); setActiveItemSearch(null); break;
            default: break;
        }
    };
    ...
    const handleScanSuccess = (barcode: string) => {
        ...
        if (foundItem) {
            ...
            const item = { ...newItems[indexToFill], ...foundItem, qty: 1, price: foundItem.price };
            const total = (item.qty) * (item.price || 0);
            item.total = total;
            item.taxAmount = isVatEnabled ? total * (vatRate / 100) : 0;
    ...
    const handleEdit = () => {
        ...
        showModal({
            title: 'تأكيد التعديل',
            message: 'هل أنت متأكد من رغبتك في تعديل بيانات هذه الفاتورة؟ لا يمكن تعديل الكميات بعد الحفظ.',
    ...
    const handleDelete = () => {
        ...
        showModal({
            title: 'تأكيد الحذف',
            message: 'هل أنت متأكد من حذف هذه الفاتورة؟ سيتم عكس تأثيرها على المخزون.',
            onConfirm: () => {
    ...
    const inputStyle = "block w-full bg-brand-green-bg border-2 border-brand-green rounded-md shadow-sm text-black placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-brand-green focus:border-brand-green py-3 px-4 disabled:bg-gray-200 disabled:cursor-not-allowed";
    const tableInputStyle = "text-center bg-transparent focus:outline-none focus:ring-1 focus:ring-brand-green rounded p-1 disabled:bg-transparent";
        ...
        <>
            <div className="bg-white p-6 rounded-lg shadow">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
                             <div className="md:col-span-1">
                                 <div className="relative bg-brand-green-bg border-2 border-brand-green rounded-md p-1 flex items-center">
                                    <button onClick={() => setPaymentMethod('cash')} className={`w-1/2 py-2 rounded ${paymentMethod === 'cash' ? 'bg-brand-green text-white shadow' : 'text-gray-600'} transition-all duration-200`} disabled={isReadOnly}>نقداً</button>
                                    <button onClick={() => setPaymentMethod('credit')} className={`w-1/2 py-2 rounded ${paymentMethod === 'credit' ? 'bg-brand-green text-white shadow' : 'text-gray-600'} transition-all duration-200`} disabled={isReadOnly}>آجل</button>
                                </div>
                             </div>
                            <input type="text" placeholder="رقم الفاتورة" className={inputStyle} value={invoiceDetails.invoiceNumber} onChange={e => setInvoiceDetails({...invoiceDetails, invoiceNumber: e.target.value})} readOnly={currentIndex > -1 || isReadOnly} />
                            <input type="date" className={inputStyle} value={invoiceDetails.invoiceDate} onChange={e => setInvoiceDetails({...invoiceDetails, invoiceDate: e.target.value})} disabled={isReadOnly}/>
                             <div className="relative" ref={supplierRef}>
                                <input type="text" placeholder="ابحث عن مورد..." className={inputStyle} value={supplierQuery} onChange={(e) => { setSupplierQuery(e.target.value); setIsSupplierDropdownOpen(true); setSelectedSupplier(null); }} onFocus={() => setIsSupplierDropdownOpen(true)} disabled={isReadOnly}/>
                                {isSupplierDropdownOpen && !isReadOnly && (
                                    <div className="absolute z-20 w-full mt-1 bg-white border border-gray-300 rounded-md shadow-lg max-h-60 overflow-y-auto">
                                        {filteredSuppliers.map(supplier => (<div key={supplier.id} onClick={() => handleSelectSupplier(supplier)} className="p-2 cursor-pointer hover:bg-brand-green-bg">{supplier.name}</div>))}
                                    </div>
                                )}
                            </div>
                        </div>
                         {paymentMethod === 'cash' && (
                            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4 items-end">
                                <div className="md:col-start-2">
                                     ...
                                     <select value={paymentTargetType} onChange={e => setPaymentTargetType(e.target.value as 'safe' | 'bank')} className={inputStyle} disabled={isReadOnly}>
                                ...
                                <div>
                                     <label className="block text-sm font-medium text-gray-700 mb-1">
                                         {paymentTargetType === 'safe' ? 'اختر الخزنة' : 'اختر البنك'}
                                     </label>
                                     <select value={paymentTargetId || ''} onChange={e => setPaymentTargetId(Number(e.target.value))} className={inputStyle} disabled={isReadOnly}>
                                         <option value="">اختر...</option>
                                         {(paymentTargetType === 'safe' ? safes : banks).map(target => (
                ...
                <div className="overflow-x-auto my-4 border-2 border-brand-green rounded-lg">
                    <table className="min-w-full border-collapse">
                        <thead className="bg-brand-green text-white">
                            <tr>
                                <th className="px-2 py-3 w-10 text-center text-sm font-semibold uppercase border border-green-300">م</th>
                                <th className="px-2 py-3 w-24 text-right text-sm font-semibold uppercase border border-green-300">رقم الصنف</th>
                                <th className="px-2 py-3 w-2/5 text-right text-sm font-semibold uppercase border border-green-300">الصنف</th>
                                <th className="px-2 py-3 w-20 text-center text-sm font-semibold uppercase border border-green-300">الوحدة</th>
                                <th className="px-2 py-3 text-center text-sm font-semibold uppercase border border-green-300" style={{minWidth: '100px'}}>الكمية</th>
                                <th className="px-2 py-3 text-center text-sm font-semibold uppercase border border-green-300" style={{minWidth: '100px'}}>السعر</th>
                                {isVatEnabled && <th className="px-2 py-3 w-36 text-center text-sm font-semibold uppercase border border-green-300">مبلغ الضريبة</th>}
                                <th className="px-2 py-3 w-36 text-center text-sm font-semibold uppercase border border-green-300">الاجمالي</th>
                        ...
                        <tbody ref={itemSearchRef} className="divide-y divide-gray-300">
                            {items.map((item, index) => (
                                 <tr key={index} className="hover:bg-brand-green-bg transition-colors duration-150">
                                    <td className="p-2 align-middle text-center border-x border-gray-300">{index + 1}</td>
                                    <td className="p-2 align-middle border-x border-gray-300"><input type="text" value={item.id} onChange={(e) => handleItemChange(index, 'id', e.target.value)} className={tableInputStyle + ' w-full'} disabled={isReadOnly}/></td>
                                    <td className="p-2 align-middle relative border-x border-gray-300">
                                        <div className="flex items-center">
                                            <input type="text" placeholder="ابحث عن صنف..." value={item.name} onChange={(e) => handleItemChange(index, 'name', e.target.value)} onFocus={() => setActiveItemSearch({ index, query: item.name })} onKeyDown={handleItemSearchKeyDown} className="bg-transparent w-full focus:outline-none p-1" disabled={isReadOnly}/>
                                            <button type="button" onClick={() => { setEditingItemIndex(index); setIsItemModalOpen(true); }} className="p-1 text-gray-400 hover:text-brand-green" disabled={isReadOnly}><ListIcon className="w-5 h-5" /></button>
                                        </div>
                                         {activeItemSearch && activeItemSearch.index === index && filteredItems.length > 0 && !isReadOnly &&(
                                            <div className="absolute z-10 w-full mt-1 bg-white border border-gray-300 rounded-md shadow-lg">
                                                {filteredItems.map((result, idx) => ( <div key={result.id} onClick={() => handleSelectItem(index, result)} className={`p-2 cursor-pointer ${idx === highlightedIndex ? 'bg-brand-green text-white' : 'hover:bg-brand-green-bg'}`} onMouseEnter={() => setHighlightedIndex(idx)}>{result.name} ({result.id})</div>))}
                                            </div>
                                        )}
                                    </td>
                                    <td className="p-2 align-middle border-x border-gray-300"><input type="text" value={item.unit} onChange={(e) => handleItemChange(index, 'unit', e.target.value)} className={tableInputStyle + ' w-full'} disabled={isReadOnly}/></td>
                                    <td className="p-2 align-middle border-x border-gray-300"><input type="number" value={item.qty} onChange={(e) => { handleItemChange(index, 'qty', parseFloat(e.target.value)); autosizeInput(e.target); }} ref={el => { if(el) qtyInputRefs.current[index] = el; }} className={tableInputStyle} disabled={isReadOnly}/></td>
                                    <td className="p-2 align-middle border-x border-gray-300"><input type="number" value={item.price} onChange={(e) => { handleItemChange(index, 'price', parseFloat(e.target.value)); autosizeInput(e.target);}} ref={el => { if(el) priceInputRefs.current[index] = el; }} className={tableInputStyle} disabled={isReadOnly}/></td>
                                    {isVatEnabled && <td className="p-2 align-middle text-center border-x border-gray-300">{item.taxAmount.toFixed(2)}</td>}
                                    <td className="p-2 align-middle text-center border-x border-gray-300">{item.total.toFixed(2)}</td>
                                    <td className="p-2 align-middle text-center border-x border-gray-300"><button onClick={() => handleRemoveItem(index)} className="text-red-500 p-1 rounded-full hover:bg-red-100 hover:text-red-700 disabled:text-gray-400 disabled:hover:bg-transparent" disabled={isReadOnly}><TrashIcon className="w-5 h-5" /></button></td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
                ...
                <div className="flex items-center gap-2 mb-4">
                    <button onClick={handleAddItem} className="px-4 py-2 bg-gray-200 text-brand-dark rounded-md hover:bg-gray-300 font-semibold disabled:bg-gray-300 disabled:cursor-not-allowed" disabled={isReadOnly}>اضافة سطر</button>
                    <button type="button" onClick={() => setIsScannerOpen(true)} className="px-4 py-2 bg-brand-green text-white rounded-md hover:bg-green-700 font-semibold flex items-center gap-2 disabled:bg-gray-400 disabled:cursor-not-allowed" disabled={isReadOnly}>
                        ...
                        <div className="w-full md:w-2/5">
                           <div className="border-2 border-brand-green rounded-lg bg-white shadow-inner">
                                <div className="flex justify-between p-3">
                                    <span className="font-semibold text-gray-600">الاجمالي قبل الضريبة</span>
                                ...
                                <div className="flex justify-between items-center p-3 border-t-2 border-dashed border-gray-200">
                                    ...
                                    <input type="number" value={totals.discount} onChange={(e) => setTotals({...totals, discount: parseFloat(e.target.value) || 0})} className="w-28 text-left p-1 rounded border-2 border-gray-600 bg-gray-700 text-white focus:ring-brand-green focus:border-brand-green font-bold" disabled={isReadOnly}/>
                                </div>
                                {isVatEnabled && (
                    ...
                    <div className="mt-4 pt-4 border-t-2 border-dashed border-gray-300">
                     ...
                     <div className="mt-6 pt-4 border-t-2 border-gray-200 flex flex-col items-center space-y-4">
                        ...
                        <div className="flex items-center justify-center gap-2">
                             <button onClick={() => navigate(invoices.length - 1)} disabled={currentIndex >= invoices.length - 1 || invoices.length === 0} className="p-2 bg-gray-200 rounded-md hover:bg-gray-300 disabled:opacity-50">الأخير</button>
                            <button onClick={() => navigate(currentIndex + 1)} disabled={currentIndex >= invoices.length - 1 || invoices.length === 0} className="p-2 bg-gray-200 rounded-md hover:bg-gray-300 disabled:opacity-50">التالي</button>
                            <div className="px-4 py-2 bg-brand-green-bg border-2 border-brand-green rounded-md">
                                 <span className="font-bold">{currentIndex > -1 ? `${currentIndex + 1} / ${invoices.length}` : `جديد`}</span>
                            </div>
            ...
            <DataTableModal
                ...
                title="قائمة الأصناف"
                columns={[ { Header: 'الكود', accessor: 'id' }, { Header: 'الاسم', accessor: 'name' }, { Header: 'الرصيد', accessor: 'stock' }, { Header: 'الوحدة', accessor: 'unit' }, { Header: 'السعر', accessor: 'price' } ]}
                data={allItems}
            ...
            />
             <DataTableModal
                ...
                columns={[ { Header: 'الرقم', accessor: 'id' }, { Header: 'التاريخ', accessor: 'date' }, { Header: 'المورد', accessor: 'supplier' }, { Header: 'الصافي', accessor: 'total' } ]}
                data={invoices.map(inv => ({id: inv.id, date: inv.date, supplier: inv.customerOrSupplier?.name || '-', total: inv.totals.net.toFixed(2)}))}
