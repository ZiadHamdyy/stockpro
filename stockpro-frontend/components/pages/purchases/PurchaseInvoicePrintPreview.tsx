...
interface PurchaseInvoicePrintPreviewProps {
    ...
    invoiceData: {
        ...
        details: { invoiceNumber: string; invoiceDate: string; userName: string; branchName: string; };
    };
}

const PurchaseInvoicePrintPreview: React.FC<PurchaseInvoicePrintPreviewProps> = ({ isOpen, onClose, invoiceData }) => {
    if (!isOpen) return null;

    const { companyInfo, vatRate, isVatEnabled, items, totals, paymentMethod, supplier, details } = invoiceData;
        ...
        <div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex justify-center items-start p-4 overflow-y-auto" onClick={onClose}>
            <div className="bg-white rounded-lg shadow-xl w-full max-w-4xl max-h-[95vh] flex flex-col my-auto" onClick={(e) => e.stopPropagation()}>
                <div className="p-4 border-b flex justify-between items-center print:hidden bg-gray-50 rounded-t-lg">
                    <h2 className="text-xl font-bold text-brand-dark">معاينة طباعة الفاتورة</h2>
                    ...
                    <div id="printable-invoice" className="p-8 bg-white">
                        <header className="flex justify-between items-start pb-4 border-b-2 border-brand-green">
                             <div className="flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-gray-600">الرقم الضريبي: {companyInfo.taxNumber}</p>
                                </div>
                            </div>
                            <div className="text-left">
                                <h1 className="text-3xl font-bold text-brand-green">فاتورة ضريبية</h1>
                                <p>Tax Invoice</p>
                            </div>
                        </header>
                        ...
                        <section className="grid grid-cols-2 gap-x-8 text-sm my-6">
                            <div className="border border-gray-300 rounded-md p-3">
                                ...
                                <p><span className="font-semibold">الاسم:</span> {supplier?.name || 'مورد نقدي'}</p>
                                <p><span className="font-semibold">العنوان:</span> ___________________</p>
                                <p><span className="font-semibold">الرقم الضريبي:</span> ___________________</p>
                            </div>
                            <div className="border border-gray-300 rounded-md p-3">
                                <p><span className="font-semibold">رقم الفاتورة:</span> {details.invoiceNumber}</p>
                                <p><span className="font-semibold">تاريخ الفاتورة:</span> {details.invoiceDate}</p>
                                <p><span className="font-semibold">الفرع:</span> {details.branchName}</p>
                                <p><span className="font-semibold">الموظف:</span> {details.userName}</p>
                            </div>
                        </section>
                        ...
                        <table className="w-full text-sm border-collapse border border-gray-300">
                            <thead className="bg-brand-green text-white">
                                <tr>
                                    ...
                                    {isVatEnabled && <th className="p-2 border border-green-300">مبلغ الضريبة</th>}
                            ...
                            <tbody className="divide-y divide-gray-300">
                               {items.map((item, index) => (
                                   <tr key={index}>
                                       ...
                                       {isVatEnabled && <td className="p-2 border border-gray-300 text-center">{item.taxAmount.toFixed(2)}</td>}
                        ...
                        <section className="flex justify-between items-start mt-4 gap-4">
                            <div className="w-1/2">
                                {isVatEnabled && qrData && <img src={qrCodeUrl} alt="QR Code" className="w-28 h-28" />}
                            </div>
