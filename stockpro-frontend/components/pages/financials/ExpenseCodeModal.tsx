...
const ExpenseCodeModal: React.FC<ExpenseCodeModalProps> = ({ isOpen, onClose, onSave, codeToEdit, expenseTypes, codes }) => {
    const [codeData, setCodeData] = useState<Omit<ExpenseCode, 'id'>>({ code: '', name: '', type: ''});
    ...
    useEffect(() => {
        if (codeToEdit) {
            ...
            const existingNumbers = codes
                .map(c => c.code.startsWith('EC-') ? parseInt(c.code.substring(3), 10) : NaN)
                .filter(n => !isNaN(n));
        ...
        <div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex justify-center items-center p-4" onClick={onClose}>
            <div className="bg-white rounded-lg shadow-xl w-full max-w-md" onClick={(e) => e.stopPropagation()}>
                <div className="p-4 border-b border-gray-200">
                    <h2 className="text-xl font-bold text-brand-dark">{codeToEdit ? 'تعديل بند مصروف' : 'اضافة بند مصروف جديد'}</h2>
                </div>
                <form onSubmit={handleSubmit}>
                    <div className="p-6 space-y-4">
                        ...
                        <div>
                            <label htmlFor="name" className="block text-sm font-medium text-gray-700">اسم البند</label>
                         ...
                         <div>
                            ...
                            <select id="type" name="type" value={codeData.type} onChange={handleChange} className={inputStyle} required>
