...
const ExpensesList: React.FC<ExpensesListProps> = ({ title, expenses, onDelete }) => {
    ...
    const handleExcelExport = () => {
        const dataToExport = expenses.map(({ code, expenseCodeName, expenseCodeType, amount, date }, index) => ({
            'م': index + 1,
            'الكود': code,
            'التاريخ': date,
    ...
    const handlePdfExport = () => {
        const head = [['المبلغ', 'نوع المصروف', 'اسم البند', 'التاريخ', 'الكود', 'م']];
        ...
        <div className="bg-white p-6 rounded-lg shadow">
            ...
            <div className="flex justify-between items-center mb-4 no-print">
                <div className="relative">
                    ...
                    <input type="text" placeholder="بحث عن مصروف..." className={inputStyle} />
                </div>
                ...
                <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-brand-blue">
                        <tr>
                            ...
                            <th className="px-6 py-3 text-right text-sm font-semibold text-white uppercase">اسم البند</th>
                            <th className="px-6 py-3 text-right text-sm font-semibold text-white uppercase">نوع المصروف</th>
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                        {expenses.map((expense, index) => (
                            <tr key={expense.id} className="hover:bg-brand-blue-bg">
                                ...
                                <td className="px-6 py-4 font-medium text-brand-dark">{expense.code}</td>
                                ...
                                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium no-print">
                                    <button onClick={() => handleDeleteClick(expense.id, expense.code)} className="text-red-600 hover:text-red-900 font-semibold">حذف</button>
                                </td>
                            </tr>
                        ))}
                    </tbody>
