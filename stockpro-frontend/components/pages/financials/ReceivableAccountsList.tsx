...
const ReceivableAccountsList: React.FC<Props> = ({ title, onNavigate }) => {
  ...
  const { data: accounts = [], isLoading, error } = useGetReceivableAccountsQuery();
  ...
  const handleExcelExport = () => {
    ...
    exportToExcel(dataToExport, "قائمة-الأرصدة-المدينة");
  };
  ...
  return (
    <>
      ...
      <div className="flex justify-between items-center mb-4 border-b pb-4 no-print">
        ...
        <PermissionWrapper requiredPermission={buildPermission(Resources.RECEIVABLE_ACCOUNTS, Actions.CREATE)} fallback={<button disabled className="px-6 py-2 bg-gray-400 text-white rounded-md cursor-not-allowed opacity-50 font-semibold transition-colors">إضافة حساب جديد</button>}>
          <button onClick={() => navigate("/financials/receivable-accounts/add")} className="px-6 py-2 bg-brand-green text-white rounded-md hover:bg-green-700 font-semibold transition-colors">إضافة حساب جديد</button>
        </PermissionWrapper>
      </div>
      ...
      <div className="flex justify-between items-center mb-4 no-print">
        ...
        <div className="flex items-center gap-2">
          <PermissionWrapper requiredPermission={buildPermission(Resources.RECEIVABLE_ACCOUNTS, Actions.PRINT)} fallback={<button disabled title="تصدير Excel" className="p-3 border-2 border-gray-200 rounded-md cursor-not-allowed opacity-50"><ExcelIcon className="w-6 h-6" /></button>}>
          ...
          <PermissionWrapper requiredPermission={buildPermission(Resources.RECEIVABLE_ACCOUNTS, Actions.PRINT)} fallback={<button disabled title="تصدير PDF" className="p-3 border-2 border-gray-200 rounded-md cursor-not-allowed opacity-50"><PdfIcon className="w-6 h-6" /></button>}>
          ...
          <PermissionWrapper requiredPermission={buildPermission(Resources.RECEIVABLE_ACCOUNTS, Actions.PRINT)} fallback={<button disabled title="طباعة" className="p-3 border-2 border-gray-200 rounded-md cursor-not-allowed opacity-50"><PrintIcon className="w-6 h-6" /></button>}>
...
export default ReceivableAccountsList;
