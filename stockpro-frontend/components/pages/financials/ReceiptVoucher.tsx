...
import type { CompanyInfo, Voucher, Customer, Supplier, CurrentAccount, VoucherEntity, User, Safe, Bank } from '../../../types';
...
const ReceiptVoucher: React.FC<ReceiptVoucherProps> = ({ title, companyInfo, vouchers, onSave, onDelete, customers, suppliers, currentAccounts, currentUser, safes, banks, viewingId, onClearViewingId }) => {
    ...
    const inputStyle = "mt-1 block w-full bg-brand-blue-bg border-2 border-brand-blue rounded-md shadow-sm text-black placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-brand-blue focus:border-brand-blue py-3 px-4 disabled:bg-gray-200 disabled:cursor-not-allowed";
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div>
                     ...
                     <div className="md:col-span-3 grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label className="block text-sm font-medium text-gray-700">استلمنا من</label>
                    ...
                    <div className="md:col-span-3">
                        ...
                        <textarea value={voucherData.description} onChange={e => setVoucherData(prev => ({...prev, description: e.target.value}))} className={inputStyle + " h-24"} placeholder="تفاصيل القبض" disabled={isReadOnly}/>
                    </div>
                    <div className="md:col-span-2 grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <div className="relative mt-1 bg-brand-blue-bg border-2 border-brand-blue rounded-md p-1 flex items-center">
                                <button onClick={() => setVoucherData(prev => ({...prev, paymentMethod: 'safe'}))} className={`w-1/2 py-2 rounded ${voucherData.paymentMethod === 'safe' ? 'bg-brand-blue text-white shadow' : 'text-gray-600'} transition-all duration-200`} disabled={isReadOnly}>نقداً (خزنة)</button>
                                <button onClick={() => setVoucherData(prev => ({...prev, paymentMethod: 'bank'}))} className={`w-1/2 py-2 rounded ${voucherData.paymentMethod === 'bank' ? 'bg-brand-blue text-white shadow' : 'text-gray-600'} transition-all duration-200`} disabled={isReadOnly}>شيك (بنك)</button>
                            </div>
                        </div>
                         <div>
