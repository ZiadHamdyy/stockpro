...
const ReceiptVoucher: React.FC<ReceiptVoucherProps> = ({ title }) => {
  ...
  const entityStats = useMemo(() => {
    ...
    // Logic for Revenue Codes
      ...
      // Revenue codes don't have currentBalance, so calculate total collected
  ...
  // Show/Hide Bar logic
  useEffect(() => {
    if (voucherData.entity.id && entityStats && !isReadOnly && !isPreviewOpen) {
      setShowInfoBar(true);
    } else {
      setShowInfoBar(false);
    }
  }, [voucherData.entity.id, entityStats, isReadOnly, isPreviewOpen]);
  ...
  useEffect(() => {
    ...
    if (currentEntityId && currentEntityId !== previousEntityId && entityStats && !isReadOnly && !isPreviewOpen) {
      setShowInfoBar(true);
    }
