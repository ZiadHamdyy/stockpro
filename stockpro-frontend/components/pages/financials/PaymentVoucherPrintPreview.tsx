...
const PaymentVoucherPrintPreview: React.FC<PaymentVoucherPrintPreviewProps> = ({ isOpen, onClose, voucherData }) => {
    if (!isOpen) return null;

    const { companyInfo, number, date, amount, paidTo, description, userName, branchName } = voucherData;
        ...
        <div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex justify-center items-start p-4 overflow-y-auto" onClick={onClose}>
            <div className="bg-white rounded-lg shadow-xl w-full max-w-3xl max-h-[95vh] flex flex-col my-auto" onClick={(e) => e.stopPropagation()}>
                <div className="p-4 border-b flex justify-between items-center print:hidden bg-gray-50 rounded-t-lg">
                    <h2 className="text-xl font-bold text-brand-dark">معاينة طباعة سند الصرف</h2>
                    <div className="flex items-center gap-2">
                        <button onClick={handlePrint} className="px-4 py-2 bg-brand-green text-white rounded-md hover:bg-green-700 font-semibold flex items-center">
                        ...
                        <div className="border-2 border-brand-green h-full p-4 flex flex-col justify-between rounded-lg">
                            <header className="flex justify-between items-center pb-2 border-b-2 border-brand-green">
                                ...
                                <div className="text-center">
                                    <h1 className="text-2xl font-bold text-brand-green">سند صرف</h1>
                            ...
                            <main className="flex-grow my-4 space-y-3 text-base">
                                <div className="flex items-center">
                                    ...
                                    <span className="border-b-2 border-dotted border-gray-400 flex-grow px-2">{paidTo}</span>
                                </div>
                                <div className="flex items-center">
