...
const Safes: React.FC<SafesProps> = ({ title, safes, branches, onSave, onDelete }) => {
            ...
            <div className="bg-white p-6 rounded-lg shadow">
                ...
                <div className="flex justify-between items-center mb-6 no-print">
                    <div className="relative">
                        ...
                        <input type="text" placeholder="بحث عن خزنة..." className={inputStyle} />
                    </div>
                    <div>
                        <button onClick={() => handleOpenModal()} className="px-6 py-3 bg-brand-blue text-white rounded-md hover:bg-blue-800 ml-2 font-semibold">اضافة خزنة جديدة</button>
                ...
                <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-gray-200">
                        <thead className="bg-brand-blue">
                            <tr>
                                <th className="px-6 py-3 text-right text-sm font-semibold text-white uppercase">كود الخزنة</th>
                                <th className="px-6 py-3 text-right text-sm font-semibold text-white uppercase">اسم الخزنة</th>
                                <th className="px-6 py-3 text-right text-sm font-semibold text-white uppercase">الفرع</th>
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                            {safes.map((safe) => (
                                <tr key={safe.id} className="hover:bg-brand-blue-bg">
                                    ...
                                    <td className="px-6 py-4 font-medium text-brand-dark">{safe.name}</td>
                                    <td className="px-6 py-4">{safe.branch}</td>
                                    <td className="px-6 py-4 text-sm font-medium no-print">
                                        <button onClick={() => handleEditClick(safe)} className="text-brand-blue hover:text-blue-800 font-semibold ml-4">تعديل</button>
                                        <button onClick={() => handleDeleteClick(safe)} className="text-red-600 hover:text-red-900 font-semibold">حذف</button>
                                    </td>
                                </tr>
                            ))}
                        </tbody>
