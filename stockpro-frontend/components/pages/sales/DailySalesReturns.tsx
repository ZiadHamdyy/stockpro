...
const DailySalesReturns: React.FC<DailySalesReturnsProps> = ({ title }) => {
  ...
  const canSearchAllBranches = useMemo(
    () =>
      hasPermission(
        buildPermission(Resources.DAILY_SALES_RETURNS, Actions.SEARCH),
      ),
    [hasPermission],
  );
  ...
  }, [allSalesReturns, canSearchAllBranches, userBranchId]);
    ...
    return salesReturns.filter((sale) => {
      ...
      const matchesBranch = !selectedBranchId || sale.branchId === selectedBranchId;
       const matchesInvoiceType =
        !invoiceType || sale.paymentMethod?.toUpperCase() === invoiceType.toUpperCase();
      const matchesSearch =
        !searchTerm ||
  ...
  }, [salesReturns, startDate, endDate, searchTerm, selectedBranchId, invoiceType]);
            ...
            {filteredReturns.map((sale, index) => (
              <tr key={sale.id} className="hover:bg-brand-blue-bg">
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  ...
                  <span className="print:inline hidden">{sale.code}</span>
                </td>
              <td className="px-6 py-4 whitespace-nowrap">
                {getInvoiceTypeLabel(sale.paymentMethod)}
              </td>
                <td className="px-6 py-4 whitespace-nowrap">
