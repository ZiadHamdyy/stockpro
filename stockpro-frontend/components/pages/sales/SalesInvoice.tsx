  ...
  useEffect(() => {
    ...
    const handleGlobalKeyDown = (e: KeyboardEvent) => {
      ...
      if (specialKeys.includes(e.key)) {
        ...
        if (e.key === 'Enter' && barcodeBufferRef.current.length >= 4) {
          ...
          const matchedItem = allItems.find(
            (item) =>
              item.barcode &&
              item.barcode.trim().toLowerCase() === scannedValue.toLowerCase(),
          );
          ...
          if (matchedItem) {
            ...
            if (activeElement && activeElement.tagName === 'INPUT' && activeElement !== barcodeInputRef.current) {
              activeElement.value = '';
        ...
        if (e.key === 'Escape' || (e.key === 'Enter' && !isBarcodeScanningRef.current)) {
          barcodeBufferRef.current = '';
          isBarcodeScanningRef.current = false;
          if (barcodeTimerRef.current) {
            clearTimeout(barcodeTimerRef.current);
      ...
      if (e.key.length === 1) {
        ...
        const timeSinceLastKey = lastKeypressTimeRef.current > 0 
        ...
        if (timeSinceLastKey < 50 && barcodeBufferRef.current.length > 0) {
          ...
          e.preventDefault();
          e.stopPropagation();
        } else if (timeSinceLastKey < 100 && isBarcodeScanningRef.current && barcodeBufferRef.current.length > 0) {
            ...
            if (barcodeBufferRef.current.length === 1 && timeSinceLastKey < 100) {
              ...
              if (activeElement && activeElement.tagName === 'INPUT' && activeElement !== barcodeInputRef.current) {
                ...
                if (currentValue.length > 0) {
                  activeElement.value = currentValue.slice(0, -1);
                  const nativeInputValueSetter = Object.getOwnPropertyDescriptor(
                    window.HTMLInputElement.prototype,
                    'value'
                  )?.set;
                  if (nativeInputValueSetter) {
                    nativeInputValueSetter.call(activeElement, currentValue.slice(0, -1));
                    const event = new Event('input', { bubbles: true });
                    activeElement.dispatchEvent(event);
                  }
                }
        ...
        barcodeTimerRef.current = setTimeout(() => {
          ...
          if (barcodeBufferRef.current.length >= 4 && isBarcodeScanningRef.current) {
            const scannedValue = barcodeBufferRef.current.trim();
            const matchedItem = allItems.find(
              (item) =>
                item.barcode &&
