...
const SalesReturn: React.FC<SalesReturnProps> = ({ title, vatRate, isVatEnabled, companyInfo, items: allItems, customers: allCustomers, invoices, onSave, onDelete, currentUser, viewingId, onClearViewingId, safes, banks }) => {
    ...
    const filteredItems = (activeItemSearch && typeof activeItemSearch.query === 'string') ? allItems.filter(item => 
     ...
     const handleNew = () => {
        ...
        setInvoiceDetails({
            invoiceNumber: `RTN-${Math.floor(10000 + Math.random() * 90000)}`,
