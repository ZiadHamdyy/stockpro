...
interface TotalRevenuesReportProps {
  title: string;
...
const TotalRevenuesReport: React.FC<TotalRevenuesReportProps> = ({
  title,
  companyInfo,
  currentUser,
}) => {
  ...
  const canSearchAllBranches = useMemo(
    () =>
      hasPermission(
        buildPermission(Resources.TOTAL_REVENUES_REPORT, Actions.SEARCH),
      ),
    [hasPermission],
  );
  ...
  // Transform API data to match expected format - filter by revenue entity type
  const revenueVouchers = useMemo(() => {
    return (apiReceiptVouchers as any[])
      ...
      .map((voucher) => {
        ...
        return {
          ...
          revenueCode: voucher.revenueCode?.name || voucher.entityName,
  ...
  const reportData = useMemo(() => {
    ...
    filteredRevenueVouchers.forEach((voucher) => {
      const monthIndex = new Date(voucher.date).getMonth();
      const key = voucher.revenueCode;
  ...
  }, [revenueVouchers, year, selectedBranch]);
      ...
      <div id="printable-area">
        ...
        <div className="flex justify-between items-center my-4 bg-gray-50 p-3 rounded-md border-2 border-gray-200 no-print">
          <div className="flex items-center gap-4 flex-wrap">
            ...
            <select
              className="p-2 border-2 border-brand-green rounded-md focus:outline-none focus:ring-2 focus:ring-brand-green bg-brand-green-bg"
            ...
            <div className="relative" ref={yearRef}>
              <input
                ...
                className="p-2 pr-8 border-2 border-brand-green rounded-md w-32"
              />
                  ...
                  {filteredYears.length > 0 ? (
                    filteredYears.map((y) => (
                      <div
                        ...
                        className="p-2 cursor-pointer hover:bg-brand-green-bg text-center"
                      >
                        {y}
                      </div>
                    ))
          ...
          <PermissionWrapper
            requiredPermission={buildPermission(
              Resources.TOTAL_REVENUES_REPORT,
              Actions.PRINT,
            )}
            fallback={
        ...
        <div
          ...
          className="overflow-x-auto border-2 border-brand-green rounded-lg"
        >
...
export default TotalRevenuesReport;
