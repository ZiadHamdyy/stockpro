...
const TotalExpensesReport: React.FC<TotalExpensesReportProps> = ({ title, companyInfo, paymentVouchers, currentUser }) => {
    ...
    const handleViewReport = useCallback(() => {
        ...
        const expenseVouchers = paymentVouchers.filter(v => 
    ...
    return (
        <div className="bg-white p-6 rounded-lg shadow">
            <div id="printable-area">
                ...
                <div id="printable-container" className="overflow-x-auto border-2 border-brand-blue rounded-lg">
                    <table className="min-w-full divide-y divide-gray-200">
                        <thead className="bg-brand-blue">
                            <tr>
                                <th className="px-4 py-3 text-right text-sm font-semibold text-white uppercase sticky right-0 bg-brand-blue z-10">اسم البند</th>
                                {months.map(month => (
                                    <th key={month} className="px-4 py-3 text-center text-sm font-semibold text-white uppercase">{month}</th>
                                ))}
                                <th className="px-4 py-3 text-center text-sm font-semibold text-white uppercase">الإجمالي</th>
                            </tr>
                        </thead>
                            ...
                            {reportData.map((item, idx) => {
                                ...
                                return (
                                    <tr key={idx} className="hover:bg-brand-blue-bg">
                                        <td className="px-4 py-4 font-medium text-brand-dark whitespace-nowrap sticky right-0 bg-white hover:bg-brand-blue-bg z-10">{item.name}</td>
                                        {item.monthly.map((amount, index) => (
                                            <td key={index} className="px-4 py-4 text-center whitespace-nowrap">{amount > 0 ? formatNumber(amount) : '-'}</td>
                                        ))}
                                        <td className="px-4 py-4 font-bold text-center whitespace-nowrap">{formatNumber(itemTotal)}</td>
                                    </tr>
                                );
                            })}
                        </tbody>
                        <tfoot className="bg-brand-blue text-white">
                            <tr className="font-bold">
                                <td className="px-4 py-3 text-right sticky right-0 bg-brand-blue z-10">الإجمالي الشهري</td>
                                {monthlyTotals.map((total, index) => (
                                    <td key={index} className="px-4 py-3 text-center whitespace-nowrap">{formatNumber(total)}</td>
                                ))}
                                <td className="px-4 py-3 text-center whitespace-nowrap">{formatNumber(grandTotal)}</td>
                            </tr>
                        </tfoot>
                    </table>
                </div>
