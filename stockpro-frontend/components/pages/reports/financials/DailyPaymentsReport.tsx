...
const DailyPaymentsReport: React.FC<DailyPaymentsReportProps> = ({
  ...
  const canSearchAllBranches = useMemo(
    () =>
      hasPermission(
        buildPermission(Resources.DAILY_PAYMENTS_REPORT, Actions.SEARCH),
      ),
    [hasPermission],
  );
  ...
  const filteredVouchers = useMemo(() => {
    const filtered = paymentVouchers.filter((voucher) => {
    ...
    return filtered;
  }, [paymentVouchers, startDate, endDate, selectedBranch, normalizeDate]);
                  ...
                  <td className="px-6 py-4">
                    <button
                      ...
                      onClick={(e) => {
                        ...
                        e.stopPropagation();
                        if (voucher.id) {
                          const url = `/financials/payment-voucher?voucherId=${encodeURIComponent(voucher.id)}`;
