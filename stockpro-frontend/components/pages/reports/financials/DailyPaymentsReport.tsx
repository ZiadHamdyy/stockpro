...
const DailyPaymentsReport: React.FC<DailyPaymentsReportProps> = ({ title, companyInfo, paymentVouchers, currentUser }) => {
    const currentYear = new Date().getFullYear();
    const [startDate, setStartDate] = useState(`${currentYear}-01-01`);
    const [endDate, setEndDate] = useState(new Date().toISOString().substring(0, 10));
    
    const totals = paymentVouchers.reduce((acc, voucher) => {
                    ...
                    <table className="min-w-full divide-y divide-gray-200">
                        <thead className="bg-brand-blue">
                            <tr>
                                ...
                                <th className="px-6 py-3 text-right text-sm font-semibold text-white uppercase">صرفنا لـ</th>
