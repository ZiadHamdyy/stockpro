...
interface PayableAccountStatementReportProps {
  title: string;
...
const PayableAccountStatementReport: React.FC<
  PayableAccountStatementReportProps
> = ({ title, companyInfo, receiptVouchers: propReceiptVouchers, paymentVouchers: propPaymentVouchers, currentUser }) => {
  ...
  const canSearchAllBranches = useMemo(
    () =>
      hasPermission(
        buildPermission(Resources.PAYABLE_ACCOUNT_STATEMENT_REPORT, Actions.SEARCH),
      ),
    [hasPermission],
  );
  ...
  const isLoading = payableAccountsLoading || branchesLoading || receiptVouchersLoading || paymentVouchersLoading;
  ...
  useEffect(() => {
    if (payableAccounts.length > 0 && !selectedAccountId) {
      setSelectedAccountId(payableAccounts[0].id.toString());
    }
  }, [payableAccounts, selectedAccountId]);

  const selectedAccount = useMemo(
    () => payableAccounts.find((c) => c.id.toString() === selectedAccountId),
    [payableAccounts, selectedAccountId],
  );
    ...
    transformedPaymentVouchers.forEach((v) => {
      ...
      if (
        ...
        if (branchMatch) {
          transactions.push({
            date: v.date,
            description: buildDescription(v, "payment", "payable_account"),
            ref: v.code || v.id,
            voucherId: v.id,
            voucherType: "payment",
    ...
    transformedReceiptVouchers.forEach((v) => {
      ...
      if (
        ...
        if (branchMatch) {
          transactions.push({
            date: v.date,
            description: buildDescription(v, "receipt", "payable_account"),
            ref: v.code || v.id,
            voucherId: v.id,
            voucherType: "receipt",
            branchName: v.branchName,
  ...
  const handleExcelExport = () => {
    ...
    exportToExcel(dataToExport, `كشف_حساب_دائن_${selectedAccount?.name || "جميع_الحسابات"}`);
  };
        ...
        <div className="flex justify-between items-center my-4 bg-gray-50 p-3 rounded-md border-2 border-gray-200 no-print">
          ...
          <PermissionWrapper
            requiredPermission={buildPermission(
              Resources.PAYABLE_ACCOUNT_STATEMENT_REPORT,
              Actions.PRINT,
            )}
            fallback={
...
export default PayableAccountStatementReport;
