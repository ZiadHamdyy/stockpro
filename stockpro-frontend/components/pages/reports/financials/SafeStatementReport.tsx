...
const SafeStatementReport: React.FC<SafeStatementReportProps> = ({ title, companyInfo, safes, receiptVouchers, paymentVouchers, currentUser }) => {
    const currentYear = new Date().getFullYear();
    const [startDate, setStartDate] = useState(`${currentYear}-01-01`);
    const [endDate, setEndDate] = useState(`${currentYear}-12-31`);
    const [selectedSafeId, setSelectedSafeId] = useState<string | null>(safes.length > 0 ? safes[0].id.toString() : null);

    const selectedSafe = useMemo(() => safes.find(s => s.id.toString() === selectedSafeId), [safes, selectedSafeId]);
    const selectedSafeName = selectedSafe?.name || 'غير محدد';

    const openingBalance = useMemo(() => {
        if (!selectedSafe) return 0;
        const receiptsBefore = receiptVouchers.filter(v => v.paymentMethod === 'safe' && v.safeOrBankId === selectedSafe.id && v.date < startDate).reduce((sum, v) => sum + v.amount, 0);
        const paymentsBefore = paymentVouchers.filter(v => v.paymentMethod === 'safe' && v.safeOrBankId === selectedSafe.id && v.date < startDate).reduce((sum, v) => sum + v.amount, 0);
        return selectedSafe.openingBalance + receiptsBefore - paymentsBefore;
    }, [selectedSafe, receiptVouchers, paymentVouchers, startDate]);
    ...
    const reportData = useMemo(() => {
        ...
        receiptVouchers.forEach(v => {
            if(v.paymentMethod === 'safe' && v.safeOrBankId === safeId && v.date >= startDate && v.date <= endDate) {
    ...
    }, [selectedSafeId, receiptVouchers, paymentVouchers, startDate, endDate, openingBalance]);
        ...
        <div className="bg-white p-6 rounded-lg shadow">
            <div id="printable-area">
                ...
                <div className="px-6 py-2 text-sm print:block hidden border-t-2 mt-2 space-y-1">
                    <p><strong>الخزينة:</strong> {selectedSafeName}</p>
                ...
                <div className="flex justify-between items-center my-4 bg-gray-50 p-3 rounded-md border-2 border-gray-200 no-print">
                    <div className="flex items-center gap-4">
                        <select className={inputStyle} value={selectedSafeId || ''} onChange={e => setSelectedSafeId(e.target.value)}>
                    ...
                    <table className="min-w-full divide-y divide-gray-200">
                        <thead className="bg-brand-blue">
                            <tr>
                                ...
                                <th className="px-6 py-3 text-right text-sm font-semibold text-white uppercase">مدين (وارد)</th>
                                <th className="px-6 py-3 text-right text-sm font-semibold text-white uppercase">دائن (صادر)</th>
