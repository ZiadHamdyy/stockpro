...
const DailyCollectionsReport: React.FC<DailyCollectionsReportProps> = ({ title, companyInfo, receiptVouchers, currentUser }) => {
    const currentYear = new Date().getFullYear();
    const [startDate, setStartDate] = useState(`${currentYear}-01-01`);
    const [endDate, setEndDate] = useState(new Date().toISOString().substring(0, 10));

    const totals = receiptVouchers.reduce((acc, voucher) => {
                ...
                <div className="overflow-x-auto border-2 border-brand-blue rounded-lg">
                    <table className="min-w-full divide-y divide-gray-200">
                        <thead className="bg-brand-blue">
                            <tr>
                                <th className="px-6 py-3 text-right text-sm font-semibold text-white uppercase">م</th>
                                <th className="px-6 py-3 text-right text-sm font-semibold text-white uppercase">التاريخ</th>
                                <th className="px-6 py-3 text-right text-sm font-semibold text-white uppercase">رقم السند</th>
                                <th className="px-6 py-3 text-right text-sm font-semibold text-white uppercase">قبضنا من</th>
                                <th className="px-6 py-3 text-right text-sm font-semibold text-white uppercase">النوع</th>
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                            {receiptVouchers.map((voucher, index) => (
                                <tr key={voucher.id} className="hover:bg-brand-blue-bg">
                                    ...
                                    <td className="px-6 py-4 font-medium text-brand-dark">{voucher.id}</td>
                                    <td className="px-6 py-4">{voucher.entity.name}</td>
                                    <td className="px-6 py-4">{voucher.paymentMethod === 'safe' ? 'نقداً' : 'بنك'}</td>
                                    <td className="px-6 py-4 font-bold">{formatNumber(voucher.amount)}</td>
                        ...
                        <tfoot className="bg-gray-100">
                            <tr className="font-bold text-brand-dark">
                                <td colSpan={5} className="px-6 py-3 text-right">الإجمالي</td>
                                <td className="px-6 py-3 text-right">{formatNumber(totals.amount)}</td>
                                <td></td>
                            </tr>
                        </tfoot>
                    </table>
