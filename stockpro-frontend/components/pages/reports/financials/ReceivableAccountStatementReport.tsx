...
interface ReceivableAccountStatementReportProps {
  title: string;
...
const ReceivableAccountStatementReport: React.FC<
  ReceivableAccountStatementReportProps
> = ({ title, companyInfo, receiptVouchers: propReceiptVouchers, paymentVouchers: propPaymentVouchers, currentUser }) => {
  ...
  const canSearchAllBranches = useMemo(
    () =>
      hasPermission(
        buildPermission(Resources.RECEIVABLE_ACCOUNT_STATEMENT_REPORT, Actions.SEARCH),
      ),
    [hasPermission],
  );
  ...
  const receivableAccounts = useMemo(() => {
    return (apiReceivableAccounts as any[]).map((account) => ({
      ...account,
  ...
  const isLoading = receivableAccountsLoading || branchesLoading || receiptVouchersLoading || paymentVouchersLoading;
  ...
  useEffect(() => {
    if (receivableAccounts.length > 0 && !selectedAccountId) {
      setSelectedAccountId(receivableAccounts[0].id.toString());
    }
  }, [receivableAccounts, selectedAccountId]);

  const selectedAccount = useMemo(
    () => receivableAccounts.find((c) => c.id.toString() === selectedAccountId),
    [receivableAccounts, selectedAccountId],
  );
        ...
        <div className="flex justify-between items-center my-4 bg-gray-50 p-3 rounded-md border-2 border-gray-200 no-print">
          ...
          <PermissionWrapper
            requiredPermission={buildPermission(
              Resources.RECEIVABLE_ACCOUNT_STATEMENT_REPORT,
              Actions.PRINT,
            )}
            fallback={
...
export default ReceivableAccountStatementReport;
