...
import type { CompanyInfo, Item, User, Invoice, StoreReceiptVoucher, StoreIssueVoucher, StoreTransferVoucher, Branch, Store } from '../../../../types';
...
const ItemMovementReport: React.FC<ItemMovementReportProps> = ({ 
    title, companyInfo, items, 
    salesInvoices, purchaseInvoices, salesReturns, purchaseReturns, 
    storeReceiptVouchers, storeIssueVouchers, storeTransferVouchers, 
    onNavigate, currentUser, branches, stores
}) => {
    const currentYear = new Date().getFullYear();
    const [startDate, setStartDate] = useState(`${currentYear}-01-01`);
    const [endDate, setEndDate] = useState(new Date().toISOString().substring(0, 10));
    const [selectedItemId, setSelectedItemId] = useState<string | null>(items.length > 0 ? items[0].id.toString() : null);
    ...
    const selectedItem = useMemo(() => items.find(i => i.id.toString() === selectedItemId), [items, selectedItemId]);
    ...
    const handleViewReport = useCallback(() => {
        ...
        const adjustBalance = (txList: any[], factor: number, isInvoice: boolean) => {
            txList.forEach((tx: any) => {
                const branchMatches = selectedBranch === 'all' || (isInvoice ? tx.branchName === selectedBranch : tx.branch === selectedBranch);
        ...
        type Transaction = { date: string, type: string, ref: string, inward: number, outward: number, link: { page: string, label: string } | null };
        const transactions: Transaction[] = [];

        const processTx = (tx: any, type: string, factor: number, linkInfo: { page: string, label: string }, isInvoice: boolean) => {
            const branchMatches = selectedBranch === 'all' || (isInvoice ? tx.branchName === selectedBranch : tx.branch === selectedBranch);
        ...
        purchaseInvoices.forEach(inv => processTx(inv, 'فاتورة مشتريات', 1, { page: 'purchase_invoice', label: 'فاتورة مشتريات' }, true));
        salesReturns.forEach(inv => processTx(inv, 'مرتجع مبيعات', 1, { page: 'sales_return', label: 'مرتجع مبيعات' }, true));
        storeReceiptVouchers.forEach(v => processTx(v, 'إذن إضافة مخزن', 1, { page: 'store_receipt_voucher', label: 'إذن إضافة مخزن' }, false));
        
        salesInvoices.forEach(inv => processTx(inv, 'فاتورة مبيعات', -1, { page: 'sales_invoice', label: 'فاتورة مبيعات' }, true));
        purchaseReturns.forEach(inv => processTx(inv, 'مرتجع مشتريات', -1, { page: 'purchase_return', label: 'مرتجع مشتريات' }, true));
        storeIssueVouchers.forEach(v => processTx(v, 'إذن صرف مخزن', -1, { page: 'store_issue_voucher', label: 'إذن صرف مخزن' }, false));
        ...
        storeTransferVouchers.forEach(v => {
            if (v.date >= startDate && v.date <= endDate) {
                ...
                v.items.forEach((item: any) => {
                    if (item.id === itemCode) {
                        if (selectedBranch === 'all' || fromStore?.branch === selectedBranch) {
                            transactions.push({ date: v.date, type: `تحويل من ${v.fromStore}`, ref: v.id, inward: 0, outward: item.qty, link: { page: 'store_transfer', label: 'تحويل مخزني' } });
                        }
                        if (selectedBranch === 'all' || toStore?.branch === selectedBranch) {
                            transactions.push({ date: v.date, type: `تحويل إلى ${v.toStore}`, ref: v.id, inward: item.qty, outward: 0, link: { page: 'store_transfer', label: 'تحويل مخزني' } });
                        }
                    }
                });
            }
    ...
    }, [selectedItem, startDate, endDate, selectedBranch, salesInvoices, purchaseInvoices, salesReturns, purchaseReturns, storeReceiptVouchers, storeIssueVouchers, storeTransferVouchers, stores]);
        ...
        <div className="bg-white p-6 rounded-lg shadow">
            ...
            <div id="printable-area">
                ...
                <div className="px-6 py-2 text-sm print:block hidden border-t-2 mt-2 space-y-1">
                    <p><strong>الصنف:</strong> {selectedItemName}</p>
                ...
                <div className="flex justify-between items-center my-4 bg-gray-50 p-3 rounded-md border-2 border-gray-200 no-print">
                    <div className="flex items-center gap-4 flex-wrap">
                        ...
                        <select className={inputStyle + " w-64"} value={selectedItemId || ''} onChange={(e) => setSelectedItemId(e.target.value)}>
                            <option value="">اختر الصنف...</option>
                            {filteredSelectItems.map(item => <option key={item.id} value={item.id}>{`${item.code} - ${item.name}`}</option>)}
                        </select>
                    ...
                    <table className="min-w-full divide-y divide-gray-200">
                        <thead className="bg-brand-blue">
                            <tr>
                                ...
                                <th className="px-6 py-3 text-right text-sm font-semibold text-white uppercase">نوع الحركة</th>
                                <th className="px-6 py-3 text-right text-sm font-semibold text-white uppercase">المرجع</th>
                                <th className="px-6 py-3 text-right text-sm font-semibold text-white uppercase">وارد</th>
                                <th className="px-6 py-3 text-right text-sm font-semibold text-white uppercase">صادر</th>
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                            ...
                            {reportData.map((item, index) => (
                                <tr key={index} className="hover:bg-brand-blue-bg">
                                    ...
                                    <td className="px-6 py-4 font-medium text-brand-dark">{item.type}</td>
                                    ...
                                    <td className="px-6 py-4 text-green-600">{formatNumber(item.inward)}</td>
                                    <td className="px-6 py-4 text-red-600">{formatNumber(item.outward)}</td>
                        ...
                        <tfoot className="bg-gray-100">
                            <tr className="font-bold text-brand-dark">
                                ...
                                <td className="px-6 py-3 text-right text-green-600">{formatNumber(totalInward)}</td>
                                <td className="px-6 py-3 text-right text-red-600">{formatNumber(totalOutward)}</td>
