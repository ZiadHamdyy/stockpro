...
import type { CompanyInfo, Customer, Invoice, Voucher, Branch, User } from '../../../../types';
...
const CustomerBalanceReport: React.FC<CustomerBalanceReportProps> = ({ title, companyInfo, customers, salesInvoices, salesReturns, receiptVouchers, paymentVouchers, branches, currentUser }) => {
    ...
    const handleViewReport = useCallback(() => {
        const customerBalanceData = customers.map(customer => {
            ...
            const sales = salesInvoices
                .filter(inv => inv.customerOrSupplier?.id === customerIdStr && inv.date <= endDate)
            ...
            const receipts = receiptVouchers
                .filter(v => v.entity.type === 'customer' && v.entity.id == customerId && v.date <= endDate)
    ...
    }, [customers, salesInvoices, salesReturns, receiptVouchers, paymentVouchers, endDate]);
                ...
                <div className="overflow-x-auto border-2 border-brand-blue rounded-lg">
                    <table className="min-w-full divide-y divide-gray-200">
                        <thead className="bg-brand-blue">
                            <tr>
                                <th className="px-6 py-3 text-right text-sm font-semibold text-white uppercase">كود العميل</th>
                                <th className="px-6 py-3 text-right text-sm font-semibold text-white uppercase">اسم العميل</th>
