...
import type { CompanyInfo, Supplier, User, Invoice, Voucher } from '../../../../types';
...
const SupplierStatementReport: React.FC<SupplierStatementReportProps> = ({ title, companyInfo, suppliers, onNavigate, currentUser, purchaseInvoices, purchaseReturns, receiptVouchers, paymentVouchers }) => {
    const currentYear = new Date().getFullYear();
    const [startDate, setStartDate] = useState(`${currentYear}-01-01`);
    const [endDate, setEndDate] = useState(new Date().toISOString().substring(0, 10));
    const [supplierSearchTerm, setSupplierSearchTerm] = useState('');
    const [selectedSupplierId, setSelectedSupplierId] = useState<string | null>(suppliers.length > 0 ? suppliers[0].id.toString() : null);
    ...
    const handleViewReport = useCallback(() => {
        ...
        const purchasesBefore = purchaseInvoices.filter(i => i.customerOrSupplier?.id === supplierIdStr && i.date < startDate).reduce((sum, i) => sum + i.totals.net, 0); // Credit
        const returnsBefore = purchaseReturns.filter(i => i.customerOrSupplier?.id === supplierIdStr && i.date < startDate).reduce((sum, i) => sum + i.totals.net, 0); // Debit
        const paymentsBefore = paymentVouchers.filter(v => v.entity.type === 'supplier' && v.entity.id == supplierId && v.date < startDate).reduce((sum, v) => sum + v.amount, 0); // Debit
        const receiptsBefore = receiptVouchers.filter(v => v.entity.type === 'supplier' && v.entity.id == supplierId && v.date < startDate).reduce((sum, v) => sum + v.amount, 0); // Credit
        
        const currentOpeningBalance = selectedSupplier.openingBalance + paymentsBefore + returnsBefore - purchasesBefore - receiptsBefore;
        ...
        purchaseInvoices.forEach(inv => {
            if(inv.customerOrSupplier?.id === supplierIdStr && inv.date >= startDate && inv.date <= endDate) {
                transactions.push({ date: inv.date, description: 'فاتورة مشتريات', ref: inv.id, debit: 0, credit: inv.totals.net, link: {page: 'purchase_invoice', label: 'فاتورة مشتريات'} });
            }
        });
         receiptVouchers.forEach(v => { // Refund from supplier we cashed
            if(v.entity.type === 'supplier' && v.entity.id == supplierId && v.date >= startDate && v.date <= endDate) {
                transactions.push({ date: v.date, description: 'سند قبض (رد مبلغ)', ref: v.id, debit: 0, credit: v.amount, link: {page: 'receipt_voucher', label: 'سند قبض'} });
            }
        });
        ...
        purchaseReturns.forEach(inv => {
            if(inv.customerOrSupplier?.id === supplierIdStr && inv.date >= startDate && inv.date <= endDate) {
                transactions.push({ date: inv.date, description: 'مرتجع مشتريات', ref: inv.id, debit: inv.totals.net, credit: 0, link: {page: 'purchase_return', label: 'مرتجع مشتريات'} });
            }
        });
        paymentVouchers.forEach(v => {
            if(v.entity.type === 'supplier' && v.entity.id == supplierId && v.date >= startDate && v.date <= endDate) {
                transactions.push({ date: v.date, description: 'سند صرف', ref: v.id, debit: v.amount, credit: 0, link: {page: 'payment_voucher', label: 'سند صرف'} });
            }
        });
    ...
    }, [selectedSupplier, purchaseInvoices, purchaseReturns, paymentVouchers, receiptVouchers, startDate, endDate]);
            ...
            <div id="printable-area">
                <ReportHeader title={title} companyInfo={companyInfo} />
                 <div className="px-6 py-2 text-sm print:block hidden border-t-2 mt-2 space-y-1">
                    <p><strong>المورد:</strong> {selectedSupplierName}</p>
                ...
                <div className="flex justify-between items-center my-4 bg-gray-50 p-3 rounded-md border-2 border-gray-200 no-print">
                    <div className="flex items-center gap-4 flex-wrap">
                        <input type="text" placeholder="بحث عن مورد..." className={inputStyle + " w-48"} value={supplierSearchTerm} onChange={e => setSupplierSearchTerm(e.target.value)} />
                        <select className={inputStyle} value={selectedSupplierId || ''} onChange={e => setSelectedSupplierId(e.target.value)}>
                            <option value="">اختر المورد...</option>
                            {filteredSuppliers.map(supplier => <option key={supplier.id} value={supplier.id}>{supplier.name}</option>)}
                        </select>
