...
import type { CompanyInfo, Supplier, Invoice, Voucher, Branch, User } from '../../../../types';
...
const SupplierBalanceReport: React.FC<SupplierBalanceReportProps> = ({ title, companyInfo, suppliers, purchaseInvoices, purchaseReturns, paymentVouchers, receiptVouchers, branches, currentUser }) => {
    ...
    const handleViewReport = useCallback(() => {
        const supplierBalanceData = suppliers.map(supplier => {
            ...
            const totalPurchases = purchaseInvoices
                .filter(inv => inv.customerOrSupplier?.id === supplierIdStr && inv.date <= endDate)
            ...
            const totalPayments = paymentVouchers
                .filter(v => v.entity.type === 'supplier' && v.entity.id == supplierId && v.date <= endDate)
            ...
            const totalDebit = totalReturns + totalPayments;
            const totalCredit = totalPurchases + totalReceipts;
    ...
    }, [suppliers, purchaseInvoices, purchaseReturns, paymentVouchers, receiptVouchers, endDate]);
                ...
                <div className="overflow-x-auto border-2 border-brand-blue rounded-lg">
                    <table className="min-w-full divide-y divide-gray-200">
                        <thead className="bg-brand-blue">
                            <tr>
                                <th className="px-6 py-3 text-right text-sm font-semibold text-white uppercase">كود المورد</th>
                                <th className="px-6 py-3 text-right text-sm font-semibold text-white uppercase">اسم المورد</th>
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                            {reportData.map((item) => (
                                <tr key={item.id} className="hover:bg-brand-blue-bg">
                                    ...
                                    <td className="px-6 py-4 font-bold">{formatNumber(item.opening)}</td>
                        ...
                        <tfoot className="bg-brand-blue text-white">
                            <tr className="font-bold">
                                ...
                                <td className="px-6 py-3 text-right">{formatNumber(totals.balance)}</td>
                            </tr>
                        </tfoot>
                    </table>
                </div>
