...
const PermissionTree: React.FC<{ 
    ...
    return (
        <div className="border-b last:border-b-0">
            ...
            {isOpen && hasChildren && (
                <div>
                    {node.children?.map(child => <PermissionTree key={child.key} node={child} level={level + 1} permissions={permissions} onPermissionChange={onPermissionChange} PERMISSION_ACTIONS={PERMISSION_ACTIONS} />)}
                </div>
            )}
        </div>
    );
        ...
        return (
            <div className="bg-white p-6 rounded-lg shadow-md">
                ...
                <div className="text-center py-8">
                    <div className="text-orange-600 mb-4 text-lg">يجب تسجيل الدخول أولاً</div>
                    <div className="text-gray-600 mb-4">لا يمكن عرض إدارة الصلاحيات بدون تسجيل الدخول</div>
                    <button 
        ...
        <div className="bg-white p-6 rounded-lg shadow-md">
            ...
            {currentUser && (
                <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-md text-sm">
                    <div className="text-blue-800 font-medium">معلومات المستخدم الحالي:</div>
                    <div className="text-blue-600">الاسم: {currentUser.name || currentUser.email}</div>
                    <div className="text-blue-600">الدور: {currentUser.role?.name || 'غير محدد'}</div>
                    {!currentUser.role && (
                        <div className="mt-2">
                            <button 
                                onClick={() => window.location.reload()} 
