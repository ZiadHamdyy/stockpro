        ...
        <div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex justify-center items-center p-4" onClick={onClose}>
            <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl" onClick={(e) => e.stopPropagation()}>
                <div className="p-4 border-b border-gray-200">
                    <h2 className="text-xl font-bold text-brand-dark">{userToEdit ? 'تعديل مستخدم' : 'اضافة مستخدم جديد'}</h2>
                </div>
                <form onSubmit={handleSubmit}>
                    <div className="p-6 grid grid-cols-1 md:grid-cols-3 gap-6 items-start">
                        <div className="md:col-span-1 flex flex-col items-center">
                            ...
                            <label htmlFor="avatar-upload" className="cursor-pointer px-4 py-2 bg-gray-200 text-gray-800 text-sm font-semibold rounded-md hover:bg-gray-300">
                                اختيار صورة
                            </label>
                        ...
                        <div className="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="text" id="fullName" name="fullName" value={userData.fullName} onChange={handleChange} className={inputStyle} required />
                            </div>
                            <div>
                                <label htmlFor="username" className="block text-sm font-medium text-gray-700">اسم المستخدم</label>
                                <input type="text" id="username" name="username" value={userData.username} onChange={handleChange} className={inputStyle} required />
                            </div>
                            <div className="relative">
                                <label htmlFor="password"className="block text-sm font-medium text-gray-700">الرقم السري</label>
                                <input 
                                    ...
                                    required={!userToEdit} 
                                />
                                <button type="button" onClick={() => setIsPasswordVisible(!isPasswordVisible)} className="absolute left-3 top-9 text-gray-500 hover:text-brand-blue">
                                    {isPasswordVisible ? <EyeOffIcon /> : <EyeIcon />}
                                </button>
                            </div>
                            <div>
                                <label htmlFor="permissionGroup" className="block text-sm font-medium text-gray-700">مجموعة الصلاحيات</label>
                                <select id="permissionGroup" name="permissionGroup" value={userData.permissionGroup} onChange={handleChange} className={inputStyle}>
                                    <option>مدير</option>
                                    <option>محاسب</option>
                                    <option>بائع</option>
                            ...
                            <div className="md:col-span-2">
                                ...
                                <select id="branch" name="branch" value={userData.branch} onChange={handleChange} className={inputStyle} required>
