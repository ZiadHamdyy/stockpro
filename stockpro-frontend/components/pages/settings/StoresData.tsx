...
const StoresData: React.FC<StoresDataProps> = ({ title, stores, branches, users, onSave, onDelete }) => {
        ...
        <>
            <div className="bg-white p-6 rounded-lg shadow">
                ...
                <div className="flex justify-between items-center mb-6 no-print">
                    ...
                    <div>
                        <button onClick={() => handleOpenModal()} className="px-6 py-3 bg-brand-blue text-white rounded-md hover:bg-blue-800 ml-2 font-semibold">اضافة مخزن جديد</button>
                ...
                <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-gray-200">
                        <thead className="bg-brand-blue">
                            <tr>
                                <th className="px-6 py-3 text-right text-sm font-semibold text-white uppercase tracking-wider">كود المخزن</th>
                                <th className="px-6 py-3 text-right text-sm font-semibold text-white uppercase tracking-wider">اسم المخزن</th>
                                <th className="px-6 py-3 text-right text-sm font-semibold text-white uppercase tracking-wider">الفرع التابع له</th>
                                <th className="px-6 py-3 text-right text-sm font-semibold text-white uppercase tracking-wider">أمين المخزن</th>
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                            {filteredStores.map((store) => (
                                <tr key={store.id} className="hover:bg-brand-blue-bg">
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap font-medium text-brand-dark">{store.name}</td>
                                    <td className="px-6 py-4 whitespace-nowrap">{store.branch}</td>
                                    <td className="px-6 py-4 whitespace-nowrap">{store.manager}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium no-print">
                                        <button onClick={() => handleEditClick(store)} className="text-brand-blue hover:text-blue-800 font-semibold ml-4">تعديل</button>
                                        <button onClick={() => handleDeleteClick(store)} className="text-red-600 hover:text-red-900 font-semibold">حذف</button>
                                    </td>
                                </tr>
                            ))}
                        </tbody>
