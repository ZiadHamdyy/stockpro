...
export const usePermissions = () => {
    ...
    const [assignPermissions, { isLoading: saving }] = useAssignPermissionsMutation();
    ...
    const arabicRoles = useMemo(() => {
        ...
        return roles.map(role => ({
            ...role,
            arabicName: ENGLISH_TO_ARABIC_ROLES[role.name as keyof typeof ENGLISH_TO_ARABIC_ROLES] || role.name
        }));
    }, [roles]);
    ...
    useEffect(() => {
        if (selectedRole && roles.length > 0) {
            const englishRoleName = ARABIC_TO_ENGLISH_ROLES[selectedRole as keyof typeof ARABIC_TO_ENGLISH_ROLES];
    ...
    const handlePermissionChange = (itemKey: string, action: string, isChecked: boolean) => {
        setPermissions(prev => {
            ...
            const permissionKey = `${itemKey}-${ARABIC_TO_ENGLISH_ACTIONS[action as keyof typeof ARABIC_TO_ENGLISH_ACTIONS]}`;
    ...
    const isAllSelectedForAction = (action: string) => {
        ...
        const englishAction = ARABIC_TO_ENGLISH_ACTIONS[action as keyof typeof ARABIC_TO_ENGLISH_ACTIONS];
        return allItemKeys.every(itemKey => rolePermissions.has(`${itemKey}-${englishAction}`));
    };
    ...
    const handleSavePermissions = async () => {
        ...
        try {
            ...
            const errorMessage = err instanceof Error ? err.message : 'حدث خطأ أثناء حفظ الصلاحيات';
    ...
    return {
        ...
        // Loading states
        isLoading: rolesLoading || isLoading || saving,
