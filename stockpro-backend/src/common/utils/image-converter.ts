 ...
 */
export function bufferToDataUri(buffer: Buffer | null | any, mimeType?: string): string | null {
  if (!buffer) {
    return null;
  }
  ...
  if (Buffer.isBuffer(buffer)) {
    ...
    console.log('🔍 Converting JSON serialized Buffer, data length:', buffer.data.length);
