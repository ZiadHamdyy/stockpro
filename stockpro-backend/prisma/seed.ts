...
async function main() {
  ...
  // First create item groups
  const itemGroup1 = await prisma.itemGroup.upsert({
  ...
  const itemGroup2 = await prisma.itemGroup.upsert({
    where: { name: 'ملابس' },
    update: {},
    create: {
      name: 'ملابس',
